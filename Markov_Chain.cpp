...
Markov_Chain::Markov_Chain(std::vector<double> currentPars, int nstep, std::string filename){
    ...
    width.push_back(0/width_factor); // deltacp : 6
    ...
    for(int i=0; i<steps; i++){
        ...
        for(int j=0; j<currentPars.size(); j++) {
            ...
            if(pars[j]){
                ...
                oscObject->set_paras(j, proposedPars[j]);
                oscObject->make_sum('p',true, false);
...
void Markov_Chain::set_width(int element, double value){
