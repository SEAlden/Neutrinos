...
void Nu_Fitter::make_sum(TH1D* hist, bool oscillate){
    ...
    double E,osci_prob1,osci_prob2,osci_prob3,osci_prob4,bin_content1,bin_content2,bin_content3,bin_content4,weight;
        ...
        for ( int i = 1; i<=nbin; i++){
            ...
            bNu->SetMNS( currentPars[5], currentPars[3], currentPars[2], currentPars[4], currentPars[1], currentPars[6] , E, kSquared, -1*kNuBar );
...
double Nu_Fitter::getLLH(TH1D* hist){
    ...
    for(int j = 1; j<=nbin; j++){
        ...
        N = _input1->GetBinContent(j) +_input2->GetBinContent(j) +_input3->GetBinContent(j)+_input4->GetBinContent(j);
