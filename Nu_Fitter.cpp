...
Nu_Fitter::Nu_Fitter(int kNuBarVar, std::string path, std::string filename){
    ...
    TFile* nue = new TFile(_path.c_str()); // creates TFile object that takes histogram of a .root file in _path directory
    TH1D* t2k = (TH1D*)nue->Get(_filename.c_str()); // gets one of the 4 fluxes from _path .root file
